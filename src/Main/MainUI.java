...
public static ObrGraLent form2=new ObrGraLent();
...
public static InformationUI form15=new InformationUI();
    /**
    ...
    private void initComponents() {
        ...
        setTitle("Таблица \"Обращения граждан\"");
        ...
        jComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Дата обращения", "Номер входящего", "ФИО", "Контакты", "Краткое содержание", "Ответственное лицо", "Срок рассмотрения обращения", "Отметка об отправке ответа", " " }));
    ...
    try {
        ...
        jTable1.getColumnModel().getColumn(0).setHeaderValue("Дата сообщение"); // задаем название столбца 0
        jTable1.getColumnModel().getColumn(1).setHeaderValue("Журнал регистрации"); // задаем название столбца 1
        jTable1.getColumnModel().getColumn(2).setHeaderValue("ФИО"); // задаем название столбца 2
        jTable1.getColumnModel().getColumn(3).setHeaderValue("Контакты");
        jTable1.getColumnModel().getColumn(4).setHeaderValue("Сообщение"); 
        jTable1.getColumnModel().getColumn(5).setHeaderValue("Сотрудник"); 
        jTable1.getColumnModel().getColumn(6).setHeaderValue("Дата отправления ответа"); 
        jTable1.getColumnModel().getColumn(7).setHeaderValue("Отметка об ответе");
        for (int i=0; i<pos; i++) {
          DATA=rs.getString("date_created");jTable1.setValueAt(DATA, i, 0);
          REGLOG=rs.getString("reg_log");jTable1.setValueAt(REGLOG, i, 1);
          FIO=rs.getString("full_name");jTable1.setValueAt(FIO, i, 2);
          CONTACTS=rs.getString("contacts");jTable1.setValueAt(CONTACTS, i,3);
          MES=rs.getString("message");jTable1.setValueAt(MES, i, 4);
          EMP=rs.getString("otvet");jTable1.setValueAt(EMP, i, 5);
          DATEEND=rs.getString("date_end");jTable1.setValueAt(DATEEND, i, 6);
          GAL=rs.getString("mark");jTable1.setValueAt(GAL, i, 7);
            try {
                Date DATEEND1 = new SimpleDateFormat("yyyy-MM-dd").parse(DATEEND);
                Long Date_End = DATEEND1.getTime();
                Long Date_Now = (new Date()).getTime();
                Long diffmil = Math.abs(Date_End - Date_Now);
                Long diff = TimeUnit.DAYS.convert(diffmil, TimeUnit.MILLISECONDS);
                if(diff <= 3){
                    JOptionPane.showMessageDialog(null,"Проверьте документы на отправку");
                }
